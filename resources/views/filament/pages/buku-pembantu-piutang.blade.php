<x-filament-panels::page>
    @php
        ...
        $kredit = 0;
    @endphp
    <h2 class="text-lg font-semibold mt-0">{{ $santriName }} Periode {{ $year }}</h2>
        ...
        <div class="ml-2 mr-4">
            ...
            <select id="santri"
                ...
                @foreach ($santris as $p)
                    <option value="{{ $p->id }}" {{ $p->id == $santri ? 'selected' : '' }}>
                        {{ $p->nama }}
                    </option>
                @endforeach
            </select>
            ...
            <tbody>
                ...
                @if ($data->count() > 0)
                    ...
                    <tr
                        ...
                        {{-- saldo piutang adalah kredit - debit --}}
                        <td class="px-6 py-3 text-center" colspan="2">
                            @if ($debit - $kredit < 0)
                                -Rp{{ strrev(implode('.', str_split(strrev(strval(-1 * ($debit - $kredit))), 3))) }}
                            @else
                                Rp{{ strrev(implode('.', str_split(strrev(strval($debit - $kredit)), 3))) }}
                            @endif
                        </td>
                    </tr>
                @endif
    ...
    <script>
        document.getElementById('search').addEventListener('click', function() {
            ...
            const santri = document.querySelector('#santri').value;
            window.location.href = `?year=${year}&santri=${santri}`;
        });
    </script>
</x-filament-panels::page>
